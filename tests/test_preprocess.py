...
def sample_data(tmp_path):
    ...
    data["fault_type"] = np.random.choice(
        ["Pastry", "Z_Scratch", "K_Scratch", "Bumps"], size=n
    )
    ...
    config = {
        ...
        "model": {"type": "random_forest", "params": {"n_estimators": 10, "random_state": 42}},
        "mlflow": {"experiment_name": "test", "tracking_uri": "sqlite:///test.db"},
        "serving": {"host": "0.0.0.0", "port": 8000, "model_path": "models/model.joblib"},
        "monitoring": {"reference_data_path": str(processed_dir / "train.csv"), "drift_threshold": 0.05},
    }
